...
export type MovieFormData = Omit<Post, 'id' | 'createdAt' | 'updatedAt' | 'reviews' | 'subtitles' | 'author' | 'authorId' | 'mediaLinks' | 'favoritedBy' | 'likedBy' | 'dislikedBy' | 'genres' | 'episodes' | 'metaData' | 'type'> & {
  mediaLinks?: MediaLink[];
  genres?: string[];
  type?: string;
};
